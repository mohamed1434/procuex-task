...
const page = () => {
  return (
    <div className="w-full flex flex-col">
      <h1>Templates</h1>
      <div className="list">
        {specifications.data.sections.map((item, index) => (
          <Sections key={index} data={item} sectionNumber={index + 1} />
        ))}
        <div className="flex flex-col items-start gap-4">
          <button className="border border-[#1993FB] px-6 py-2 rounded-lg text-[#1993FB]
          ...
          <div className="flex flex-col gap-2 md:gap-0 justify-between items-cente w-full md:flex-row pb-6">
            ...
            <button className="px-24 py-2 bg-[#1993FB] text-white text-[12px] rounded-lg hover:opacity-75 duration-200">
              Save and Proceed
            </button>
          </div>
        </div>
